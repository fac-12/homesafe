...
var confirmPassword = function confirmPassword(password, confirm_password, display_error, button) {
  return password.value !== confirm_password.value ? (display_error.textContent = "The passwords do not match.", button.disabled = true) : (button.disabled = false, display_error.textContent = "");
};

var strongPassword = function strongPassword(password, display_error, button) {
  var regex = new RegExp('^(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9]).{6,}$');
  return !regex.test(password.value) ? (display_error.textContent = "Passwords must contain one uppercase letter, one number, one special case letter(!@#$&*) and must be a minimum of 6 characters.", button.disabled = true) : (button.disabled = false, display_error.textContent = "");
};

var emailValid = function emailValid(email, display_error, button) {
  var regex = new RegExp('[A-Za-z0-9._-]+@[A-Za-z0-9.-]+\\.[A-Za-z]');
  !regex.test(email.value) ? (display_error.textContent = "Please enter a valid email.", button.disabled = true) : (button.disabled = false, display_error.textContent = "");
};
...
var validPostcode = function validPostcode(postcode, display_error, button) {
  ...
  return !regex.test(postcode.value.replace(/\s/g, '')) ? (display_error.textContent = "Enter a valid UK postcode.", button.disabled = true) : (button.disabled = false, display_error.textContent = "");
};

var checkPhone = function checkPhone(phone, display_error, button) {
  var regex = new RegExp('^(\\[+44]\\s?7\\d{3}|\\(?07\\d{3}\\)?)\\s?\\d{3}\\s?\\d{3}$');
  return !regex.test(phone.value) ? (display_error.textContent = "Please enter a valid UK phone number. i.e 07934562565", button.disabled = true) : (button.disabled = false, display_error.textContent = "");
};
