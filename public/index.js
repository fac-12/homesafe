...
const confirmPassword = (password, confirm_password, display_error, button) => {
  ...
  (display_error.textContent = "The passwords do not match.",
...
const strongPassword = (password, display_error, button) => {
  ...
  return !regex.test(password.value) ? (display_error.textContent = "Passwords must contain one uppercase letter, one number, one special case letter(!@#$&*) and must be a minimum of 6 characters.", button.disabled = true) :
    (button.disabled = false,
      display_error.textContent = "");
}
...
const emailValid = (email, display_error, button) => {
  ...
  !regex.test(email.value) ? (display_error.textContent = "Please enter a valid email.", button.disabled = true) :
    (button.disabled = false,
      display_error.textContent = "");
}
...
const validPostcode = (postcode, display_error, button) => {
  ...
  return !regex.test(postcode.value.replace(/\s/g, '')) ? (display_error.textContent = "Enter a valid UK postcode.", button.disabled = true) :
    (button.disabled = false,
      display_error.textContent = "")
}
...
const checkPhone = (phone, display_error, button) => {
  ...
  return !regex.test(phone.value) ? (display_error.textContent = "Please enter a valid UK phone number. i.e 07934562565", button.disabled = true) :
    (button.disabled = false,
      display_error.textContent = "");
}
